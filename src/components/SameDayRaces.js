...
const buildMapsUrl = (origin, venue) => {
  ...
  const googleUrl = `https://www.google.com/maps/dir/${origin.lat},${
    origin.lng
  }/${
    matches && matches.length > 0
      ? matches[0].replace(/ /g, "+")
      : venue.replace(/ /g, "+")
  }/@${origin.lat},${origin.lng},10z`;
...
const DrivingDistance = compose(
  ...
  const duration = props.directions
  ...
  return props.directions.routes[0] && props.directions.routes[0].legs[0] ? (
    <div className={props.classes.directionsCell}>
      <DirectionsCar size={"small"} />
          ...
          <CardContent>
            <Paper className={classes.tableRoot}>
              <Table className={classes.table} padding={"none"}>
                ...
                <TableBody>{sameDayRaces}</TableBody>
              </Table>
            </Paper>
          </CardContent>
        </Card>
